...
function frag_edge(V, EV::Lar.ChainOp, edge_idx::Int, bigPI)
    ...
    for i in bigPI[edge_idx]
        if i != edge_idx
            intersection = Lar.CPDTGW3D8a.intersect_edges(
            	V, edge, EV[i, :])
...
function componentgraph(V, copEV, bicon_comps)
	...
	for p in 1:n
		...
		fe = CPDTGW3D8a.minimal_2cycles(V, ev)
        # exterior cycle
		shell_num = CPDTGW3D8a.get_external_cycle(V, ev, fe)
...
function planar_arrangement_2(V, copEV,bicon_comps, edge_map,
	...
	bicon_comps = CPDTGW3D8a.biconnected_components(copEV)

	# component graph
	n, containment_graph, V, EVs, boundaries, shells, shell_bboxes=CPDTGW3D8a.componentgraph(V,copEV,bicon_comps)

	copEV, FE = CPDTGW3D8a.cell_merging(
...
#planar_arrangement_1
	V,copEV,sigma,edge_map=CPDTGW3D8a.planar_arrangement_1(V,copEV,sigma,return_edge_map,multiproc)

# cleandecomposition
	if sigma.n > 0
		V,copEV=CPDTGW3D8a.cleandecomposition(V, copEV, sigma, edge_map)
	end
...
#Planar_arrangement_2
	V,copEV,FE=CPDTGW3D8a.planar_arrangement_2(V,copEV,bicon_comps,edge_map,sigma)
	if (return_edge_map)
